...
from dash import ALL, Dash, Input, Output, State, dcc, html, no_update, Patch, ctx
        ...
        dbc.Row([
            ...
            dbc.Col([
                ...
                dbc.Card([
                    dbc.CardHeader(
                        dbc.Row([
                            dbc.Col("Technique", className="fw-bold fs-5"),
                            dbc.Col(dbc.Button("Réduire / Ouvrir", id="btn-collapse-tech", color="light", size="sm", className="float-end text-success fw-bold"), width="auto")
                        ], align="center"),
                    ...
                    dbc.Collapse(
                        dbc.CardBody([
                            html.Div([
                                html.Label("Mode d'affichage :", className="fw-bold mb-2 me-3"),
                                dbc.RadioItems(id="tech-plot-mode", options=[], value="1", inline=True, className="mb-3")
                ...
                dbc.Card([
                    dbc.CardHeader(
                        dbc.Row([
                            dbc.Col("Critères", className="fw-bold fs-5"),
                            dbc.Col(dbc.Button("Réduire / Ouvrir", id="btn-collapse-crit", color="light", size="sm", className="float-end text-info fw-bold"), width="auto")
                        ], align="center"),
                    ...
                    dbc.Collapse(
                        dbc.CardBody([
                            html.Div([
                                html.Label("Mode d'affichage :", className="fw-bold mb-2 me-3"),
                                dbc.RadioItems(id="crit-plot-mode", options=[], value="1", inline=True, className="mb-3")
...
@app.callback(
    Output("collapse-tech", "is_open"),
    [Input("btn-collapse-tech", "n_clicks")],
    [State("collapse-tech", "is_open")],
)
def toggle_tech_collapse(n, is_open):
    if n:
        return not is_open
    return is_open

@app.callback(
    Output("collapse-crit", "is_open"),
    [Input("btn-collapse-crit", "n_clicks")],
    [State("collapse-crit", "is_open")],
)
def toggle_crit_collapse(n, is_open):
    if n:
        return not is_open
    return is_open
...
def handle_sliders_visibility_and_values(x_ctx, y_ctx, click_data, slider_ids, current_values):
    ...
    if click_data and "points" in click_data:
        ...
        for i, s_id in enumerate(slider_ids):
            ...
            if col_name == x_ctx and clicked_x_label in unique_vals:
                new_values[i] = unique_vals.index(clicked_x_label)
...
@app.callback(
    ...
    # prevent_initial_call supprimé ici !
)
def update_criteria_graph(x, y, z, mode, yr, click_data, slider_ids, slider_values, x_ctx_col, y_ctx_col):
    if df is None: 
        return go.Figure()

    # 1. On initialise un mapping par défaut (tous les sliders à l'index 0)
    mapping_sliders = {col: 0 for col in contexte_dispo}
    
    # Si on a des sliders réels dans la page, on prend leurs vraies valeurs
    if slider_ids:
        for sid, sval in zip(slider_ids, slider_values):
            mapping_sliders[sid["index"]] = sval
    
    # 2. Si l'utilisateur vient de cliquer sur la matrice de contexte
    ...
    # 3. Re-ordonnancement pour le Reshape matriciel
